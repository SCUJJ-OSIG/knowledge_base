...
**输入**: 可选地在 `/opsx:archive` 后指定变更名称（例如 `/opsx:archive add-auth`）。如果省略，检查是否可以从对话上下文中推断。如果模糊或含糊不清，必须提示可用的变更。
...
4. **评估 delta spec 同步状态**
   ...
   在 `openspec/changes/<name>/specs/` 检查 delta specs。如果不存在，继续而不提示同步。
